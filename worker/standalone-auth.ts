...
export async function authenticate(request: Request, env: AuthEnv): Promise<Request | Response> {
  ...
  if (protectedPath(url.pathname)) {
    const email = env.ADMIN_EMAILS?.split(",")[0]?.trim().toLowerCase();
    if (!email || !env.ADMIN_PASSWORD || env.ADMIN_PASSWORD.length < 20) return new Response("Administrator access is not configured.", {status:503});
    ...
    if (colon < 0 || decoded.slice(0,colon).toLowerCase() !== email || !await equal(decoded.slice(colon+1),env.ADMIN_PASSWORD)) return deny();
